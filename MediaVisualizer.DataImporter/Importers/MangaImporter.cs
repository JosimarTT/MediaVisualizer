...
public class MangaImporter
{
    ...
    private readonly string basePath =
        Path.Combine(StringConstants.BaseCollectionPath, StringConstants.MangaFolderPath);
    ...
    public async Task ImportData()
    {
        ...
        foreach (var folder in StringConstants.MangaFolders)
        {
            var folderPath = Path.Combine(basePath, folder);
            if (Directory.Exists(folderPath))
            {
                var files = Directory.GetFiles(folderPath, "*.*", SearchOption.AllDirectories)
                    .Where(file => StringConstants.ImageExtensions.Contains(Path.GetExtension(file)))
                    .ToList();
